...
function autoSyncToSheets() {
  ...
  // Debounce 2 detik setelah perubahan data, lalu sync
...
function toast(msg, dur, type) {
  ...
  var bg = colors[type] || '#334155';
  el.style.cssText = 'position:fixed;bottom:24px;right:20px;z-index:99999;background:'+bg+';color:#fff;'+
...
// ── STATUS BADGE — warna lengkap ─────────────────────────────────
function badgeBayar(s, sisa){
  if(s==='Lunas'||sisa===0) return '<span class="badge bg-green"><span class="dot dot-g"></span>Lunas ✓</span>';
...
function rowColorTrx(t) {
  if (!t.sisa || t.sisa <= 0) return '';
  if (t.bayar === 'DP') return 'background:rgba(245,158,11,0.05);';
...
function getLaporanData() {
  ...
  var tahun  = today.substring(0,4);

  // Populate kasir dropdown jika belum terisi
  var kasirEl = document.getElementById('lap-filter-kasir');
  if (kasirEl && kasirEl.options.length <= 1) {
    var kasirList = [...new Set(TRX.map(t=>t.kasir).filter(Boolean))];
    kasirList.forEach(function(k){
...
function renderLaporan(){
  ...
  document.getElementById('lap-stats').innerHTML=
    sc('Omzet',fmtRp(omzetBulanIni),'color:var(--blue-d)','Total penjualan','color:var(--tx2)','blue')+
    sc('Modal Vendor',fmtRp(modalVendorBulanIni),'color:var(--amber-d)','Belanja bahan/kulakan','color:var(--tx2)','amber')+
  ...
  document.getElementById('lap-brg').innerHTML=`<table><thead><tr><th>Nama Barang</th><th>Qty Terjual</th><th>Sumbangan Omzet</th></tr></thead><tbody>${brows||emptyRow(3,'📊','Belum ada penjualan.')}</tbody></table>`;
  
  var kmap={};
  dataTrxBulanIni.forEach(t=>{if(!kmap[t.kasir])kmap[t.kasir]={count:0,omzet:0};kmap[t.kasir].count++;kmap[t.kasir].omzet+=t.total;});
  var krows=Object.entries(kmap).sort((a,b)=>b[1].omzet-a[1].omzet).map(e=>`<tr><td style="font-weight:700">${e[0]}</td><td>${e[1].count} nota</td><td style="color:var(--blue);font-weight:800">${fmtRp(e[1].omzet)}</td></tr>`).join('');
  document.getElementById('lap-kar').innerHTML=`<table><thead><tr><th>Nama Kasir</th><th>Transaksi</th><th>Omzet</th></tr></thead><tbody>${krows||emptyRow(3,'👤','Tidak ada data kasir')}</tbody></table>`;

  var komMap={};
  dataTrxBulanIni.forEach(t=>{if(t.komisiNama&&t.komisiNominal>0){if(!komMap[t.komisiNama])komMap[t.komisiNama]=0;komMap[t.komisiNama]+=t.komisiNominal;}});
  var komRows=Object.entries(komMap).sort((a,b)=>b[1]-a[1]).map(e=>`<tr><td style="font-weight:700">${e[0]}</td><td style="color:var(--green-d);font-weight:800;background:var(--green-l);">${fmtRp(e[1])}</td></tr>`).join('');
  var lapKomEl=document.getElementById('lap-komisi');
  if(lapKomEl) lapKomEl.innerHTML=`<table><thead><tr><th>Nama Pekerja</th><th>Total Komisi / Tip</th></tr></thead><tbody>${komRows||emptyRow(2,'💸','Tidak ada komisi tercatat.')}</tbody></table>`;
}
...
/* ════════════════════════════════════════════════════════════════
   ✨ FITUR TAMBAHAN — semua 12 fitur
   ════════════════════════════════════════════════════════════════ */

// ── F1: SHORTCUT KEYBOARD (Desktop Only) ─────────────────────────
(function initKeyboardShortcuts() {
  ...
  document.addEventListener('keydown', function(e) {
    ...
    // ESC — tutup modal
    if (e.key === 'Escape') {
      var openMod = document.querySelector('.modal-bg.open');
      if (openMod) { openMod.classList.remove('open'); return; }
    }

    // Ctrl+P — print nota
    if (e.ctrlKey && e.key === 'p') {
      var notaMod = document.getElementById('mo-nota');
      if (notaMod && notaMod.classList.contains('open')) {
        e.preventDefault();
        if (typeof ReceiptController !== 'undefined') ReceiptController.showPrintSelector();
        else window.print();
      }
      return;
    }
    ...
    // Enter — tambah barang ke keranjang
    if (e.key === 'Enter') {
      e.preventDefault();
      var btn = document.getElementById('btn-tambah-item') ||
                document.querySelector('[onclick*="tambahKeCart"],[onclick*="tambahItem"],[onclick*="addCart"]');
      if (btn) btn.click();
      return;
    }

    // F2 — simpan/bayar transaksi
    if (e.key === 'F2') {
      e.preventDefault();
      var btnSimpan = document.getElementById('btn-simpan-trx') ||
                     document.querySelector('[onclick*="simpanTrxPage"]');
      if (btnSimpan) btnSimpan.click();
      return;
    }
  });
})();

// ── F4: BOSS WIDGETS DASHBOARD ───────────────────────────────────
function renderBossWidgets() {
  ...
  var trxHari  = TRX.filter(function(t){ return t.tgl===today; });
  var pengHari = PENGELUARAN.filter(function(p){ return p.tgl===today; });
  var trxBulan = TRX.filter(function(t){ return t.tgl&&t.tgl.startsWith(bulanIni); });

  var omzetHari  = trxHari.reduce(function(s,t){ return s+(t.dibayar||0); },0);
  var keluarHari = pengHari.reduce(function(s,p){ return s+(p.total||0); },0);
  var modalHari  = trxHari.reduce(function(s,t){ return s+(t.modal||0); },0);
  ...
  var omzetBulan = trxBulan.reduce(function(s,t){ return s+t.total; },0);
  var progress   = Math.min(100, Math.round((omzetBulan/target)*100));
  var progColor  = progress>=80 ? '#10B981' : progress>=50 ? '#F59E0B' : '#EF4444';

  el.style.display = 'block';
  el.innerHTML =
    '<div style="font-size:12px;font-weight:800;color:var(--tx3);letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">📊 Ringkasan Hari Ini</div>'+
    '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-bottom:16px;">'+
      _bossCard('💰','Omzet Hari Ini',fmtRp(omzetHari),'#3B82F6')+
      _bossCard('📈','Profit Hari Ini',fmtRp(profitHari),profitHari>=0?'#10B981':'#EF4444')+
      _bossCard('💸','Pengeluaran',fmtRp(keluarHari),'#F59E0B')+
      _bossCard('⏳','Pending',pending.length+' nota','#8B5CF6')+
      _bossCard('👤','Total Piutang',fmtRp(totalPiutang),'#EF4444')+
    '</div>'+
    '<div class="card" style="padding:14px 18px;margin-bottom:20px;">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'+
        '<div style="font-weight:800;font-size:13px;">🎯 Target Bulan Ini</div>'+
        '<div style="font-size:11px;color:var(--tx2);">'+fmtRp(omzetBulan)+' / '+fmtRp(target)+'</div>'+
      '</div>'+
      '<div style="background:var(--surf2);border-radius:99px;height:8px;overflow:hidden;">'+
        '<div style="height:100%;width:'+progress+'%;background:'+progColor+';border-radius:99px;transition:width 1.2s ease;"></div>'+
      '</div>'+
      '<div style="font-size:11px;color:var(--tx3);margin-top:5px;">'+progress+'% tercapai'+
        (progress>=100?' 🎉 Target tercapai!':(target-omzetBulan>0?' — kurang '+fmtRp(target-omzetBulan):''))+
      '</div>'+
    '</div>';
}

function _bossCard(ico,label,val,color) {
  return '<div class="card" style="padding:12px 14px;margin:0;border-top:3px solid '+color+';min-height:80px;">'+
    '<div style="font-size:16px;margin-bottom:4px;">'+ico+'</div>'+
    '<div style="font-size:10px;font-weight:700;color:var(--tx3);margin-bottom:2px;">'+label+'</div>'+
    '<div style="font-size:15px;font-weight:900;color:'+color+';font-family:var(--mono);">'+val+'</div>'+
  '</div>';
}

// ── F7: EXPORT CSV & PDF ─────────────────────────────────────────
function exportToCSV(data, filename, headers) {
  if (!data || !data.length) { toast('Tidak ada data untuk diekspor!', 2000, 'warning'); return; }
  ...
  toast('✅ File CSV berhasil diunduh!', 2500, 'success');
}
...
function exportTransaksiCSV() {
  ...
  exportToCSV(trx.map(function(t) {
    return {
      'ID':t.id,'Tanggal':t.tgl,'Pelanggan':t.pelanggan,'WA':t.wa||'',
      'Item':(t.items||[]).map(function(i){return i.barang+' x'+i.qty;}).join(' | '),
      'Total':t.total,'Dibayar':t.dibayar||0,'Sisa':t.sisa||0,
      'Status':t.bayar,'Metode':t.metode||'Cash','Kasir':t.kasir
    };
  }), 'Transaksi');
}
...
function exportPengeluaranCSV() {
  exportToCSV(PENGELUARAN.map(function(p) {
    return {'ID':p.id,'Tanggal':p.tgl,'Vendor':p.vendor||'','Keterangan':p.ket||'','Total':p.total||0,'Status':p.bayar||'','Kategori':p.kategori||''};
  }), 'Pengeluaran');
}
...
function exportToPDF(elementId, title) {
  ...
  if (!win) { toast('Pop-up diblokir browser!', 2500, 'error'); return; }
  win.document.write('<!DOCTYPE html><html><head><title>'+(title||'Laporan')+'</title>'+
    '<style>body{font-family:Arial,sans-serif;padding:20px;color:#000;}table{width:100%;border-collapse:collapse;font-size:12px;}th,td{border:1px solid #ddd;padding:6px 10px;text-align:left;}th{background:#f1f5f9;font-weight:700;}h2{margin-bottom:12px;}@page{size:A4;margin:15mm;}</style>'+
...
// ── F8: AUDIT LOG ────────────────────────────────────────────────
var AUDIT_KEY_V2 = 'abunawas_audit_v2';

function addAuditEntry(action, module, detail, before, after) {
  try {
    var logs = JSON.parse(localStorage.getItem(AUDIT_KEY_V2)||'[]');
    logs.unshift({
      id    : 'A-'+Date.now(),
      ts    : new Date().toISOString(),
      user  : curUser ? curUser.nama : 'System',
      role  : curUser ? curUser.role : '-',
      action: action,  // EDIT | CREATE | DELETE | LOGIN | LOGOUT | EXPORT
      module: module,
      detail: detail||'',
      before: before||null,
      after : after||null
    ...
    localStorage.setItem(AUDIT_KEY_V2, JSON.stringify(logs));
  } catch(e){}
}

// ── F10: OFFLINE MODE ────────────────────────────────────────────
var OFFLINE_Q_KEY = 'kasir_offline_queue_v2';

function saveOfflineQueue(trx) {
  var q = JSON.parse(localStorage.getItem(OFFLINE_Q_KEY)||'[]');
  q.push({trx:trx, ts:Date.now()});
  localStorage.setItem(OFFLINE_Q_KEY, JSON.stringify(q));
  toast('📵 Offline — transaksi disimpan lokal, auto-sync saat online.', 3000, 'warning');
}
...
function syncOfflineQueue() {
  ...
  q.forEach(function(item){
    if(!TRX.find(function(t){return t.id===item.trx.id;})) {
      TRX.unshift(item.trx); added++;
    }
  });
  localStorage.removeItem(OFFLINE_Q_KEY);
  if(added>0) {
    saveData(); syncToSheets(true);
    toast('✅ '+added+' transaksi offline berhasil disinkronkan!', 3000, 'success');
  }
}

window.addEventListener('online',  function(){ toast('🌐 Online kembali! Menyinkronkan data...', 2000, 'info'); setTimeout(syncOfflineQueue, 1500); });
window.addEventListener('offline', function(){ toast('📵 Koneksi terputus — mode offline aktif.', 2500, 'warning'); });

function updateOnlineStatus() {
  var el = document.getElementById('online-status-dot');
  if(el) { el.style.background = navigator.onLine ? '#10B981' : '#F59E0B'; el.title = navigator.onLine ? 'Online' : 'Offline'; }
}
...
var WA_TEMPLATES = {
  notaLengkap: function(t) {
    var items = (t.items||[]).map(function(i){ return '▪️ '+i.barang+' x'+i.qty+' = '+fmtRp(i.total); }).join('\n');
    return '🧾 *NOTA PESANAN — '+(TOKO.nama||'Toko')+'*\n\nID: *'+t.id+'*\nTanggal: '+t.tgl+'\nKasir: '+t.kasir+
      '\n\n📦 *DETAIL PESANAN:*\n'+items+'\n\n💰 *Total: '+fmtRp(t.total)+'*\n'+
      (t.sisa>0?'⚠️ Sisa: *'+fmtRp(t.sisa)+'*\n':'✅ Status: *LUNAS*\n')+'\nTerima kasih! 🙏';
  },
  dpMasuk: function(t) {
    return '💳 *DP DITERIMA — '+(TOKO.nama||'Toko')+'*\n\nHalo *'+t.pelanggan+'*,\n'+
      'DP sebesar *'+fmtRp(t.dibayar)+'* sudah kami terima.\n\n📋 ID: '+t.id+'\n💰 Total: '+fmtRp(t.total)+
      '\n⚠️ Sisa: *'+fmtRp(t.sisa)+'*\n\nPesanan segera diproses. Terima kasih! 🙏';
  },
  lunas: function(t) {
    return '✅ *LUNAS — '+(TOKO.nama||'Toko')+'*\n\nHalo *'+t.pelanggan+'*,\n'+
      'Pembayaran *'+fmtRp(t.total)+'* sudah kami terima LUNAS.\n📋 ID: '+t.id+'\n\nTerima kasih! 🙏';
  },
  pending: function(t) {
    return '⏳ *PEMBAYARAN PENDING*\n\nHalo *'+t.pelanggan+'*, pesanan ID *'+t.id+'* belum kami terima pembayarannya.\n'+
      '💰 Total: '+fmtRp(t.total)+'\n⚠️ Belum dibayar: *'+fmtRp(t.sisa)+'*\n\nMohon segera lakukan pembayaran. 🙏';
  },
  reminderAmbil: function(t) {
    return '📦 *REMINDER — Pesanan Siap Diambil!*\n\nHalo *'+t.pelanggan+'*, pesanan Anda sudah *SELESAI*!\n'+
      '📋 ID: '+t.id+'\n\nSegera diambil sebelum 7 hari ya kak. Terima kasih! 🙏';
  },
  reminderHutang: function(t) {
    return '⚠️ *REMINDER TAGIHAN*\n\nHalo *'+t.pelanggan+'*, masih ada tagihan yang belum diselesaikan.\n'+
      '📋 ID: '+t.id+'\n💰 Total: '+fmtRp(t.total)+'\n🔴 Sisa: *'+fmtRp(t.sisa)+'*\n\nMohon segera dilunasi. Terima kasih! 🙏';
  }
};

function kirimWATemplate(trxId, tplKey) {
  var t = TRX.find(function(x){ return x.id===trxId; });
  if(!t) { toast('Transaksi tidak ditemukan!', 2000, 'error'); return; }
  var fn = WA_TEMPLATES[tplKey];
  if(!fn) { toast('Template tidak ada!', 2000, 'error'); return; }
  var msg = fn(t);
  if(t.wa && typeof sendWA==='function') sendWA(t.wa, msg);
  else { navigator.clipboard.writeText(msg).then(function(){ toast('Pesan disalin ke clipboard!', 2500, 'info'); }).catch(function(){ alert(msg); }); }
}

// ── F9: MULTI METODE PEMBAYARAN ──────────────────────────────────
function onMetodeBayarChange(val) {
  var dpWrap = document.getElementById('extra-metode-wrap');
  if (!dpWrap) return;
  dpWrap.style.display = (val === 'DP' || val === 'OVO' || val === 'Dana' || val === 'ShopeePay') ? 'block' : 'none';
}

// ── PATCH: renderDash patch untuk boss widgets (sudah inline di renderDash)
// ── PATCH: logActivity hook ke addAuditEntry ──────────────────────
var _origLogActivity = typeof logActivity === 'function' ? logActivity : null;
if(_origLogActivity) {
  logActivity = function(aksi, modul, data) {
    _origLogActivity(aksi, modul, data);
    addAuditEntry(aksi, modul, (data&&data.label)||'', (data&&data.before)||null, (data&&data.after)||null);
  };
}
